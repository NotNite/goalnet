...
fn load_internal(config_path_vec: Vec<u8>) -> anyhow::Result<()> {
    let config_path = String::from_utf8(config_path_vec)
        .context("failed to parse config path")?;
    let config_path = PathBuf::from_str(&config_path)
        .context("failed to convert config path")?;
    let config = goalnet_common::parse(&config_path)
    ...
    let payload_directory = if config.payload.copy_build_artifacts {
        let temp_dir = std::env::temp_dir()
            .join(goalnet_common::temp_filename());

        copy_dir_all(&payload_directory, &temp_dir)
    ...
    let dll_pdcstr = PdCString::from_os_str(dll.as_os_str())
        .context("failed to convert dll path")?;
    ...
    let type_name = PdCString::from_str(&config.entrypoint.type_name)
        .context("failed to convert type name")?;
